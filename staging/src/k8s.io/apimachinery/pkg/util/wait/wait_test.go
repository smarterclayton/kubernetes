...
func Test_loopConditionUntilContext_Elapsed(t *testing.T) {
	t.Run("variable timer with jitter", func(t *testing.T) {
		var attempts int
		start := time.Now()
		if err := loopConditionUntilContext(context.Background(), Backoff{Duration: time.Millisecond, Jitter: 1.0}.Timer(), false, false, func(_ context.Context) (bool, error) {
			attempts++
			if attempts > 11 {
				t.Fatalf("should not reached 11 attempts")
			}
		...
		if duration < 10*time.Millisecond {
			t.Fatalf("total duration is less than 10 milliseconds: %v", duration)
		}
		// TODO: this may be too aggressive, but the overhead should be minor
		if duration > 21*time.Millisecond {
			t.Fatalf("total duration is more than 21 milliseconds: %v", duration)
		}
	})
	t.Run("fixed timer", func(t *testing.T) {
		var attempts int
		start := time.Now()
		if err := loopConditionUntilContext(context.Background(), Backoff{Duration: time.Millisecond}.Timer(), false, false, func(_ context.Context) (bool, error) {
			attempts++
			if attempts > 11 {
				t.Fatalf("should not reached 11 attempts")
			}
		...
		// TODO: this may be too aggressive, but the overhead should be minor
		if duration > 12*time.Millisecond {
			t.Fatalf("total duration is more than 12 milliseconds: %v", duration)
		}
	})
	t.Run("no-op timer", func(t *testing.T) {
		var attempts int
		start := time.Now()
		if err := loopConditionUntilContext(context.Background(), Backoff{Duration: 0}.Timer(), false, false, func(_ context.Context) (bool, error) {
			attempts++
			if attempts > 11 {
				t.Fatalf("should not reached 11 attempts")
			}
		...
		if duration > 1*time.Millisecond {
			t.Fatalf("total duration is more than 1 milliseconds: %v", duration)
		}
	})
}
