...
func createPodWorkers() (*podWorkers, map[types.UID][]syncPodRecord) {
	...
	w := newPodWorkers(
		...
		func(pod *v1.Pod, podStatus *kubecontainer.PodStatus, isRuntimeOnly bool, gracePeriod *int64, podStatusFn func(*v1.PodStatus)) error {
			func() {
				lock.Lock()
				defer lock.Unlock()
				processed[pod.UID] = append(processed[pod.UID], syncPodRecord{
...
func TestUpdatePodForRuntimePod(t *testing.T) {
	...
	if !updates[0].runtime || updates[0].updateType != kubetypes.SyncPodKill || updates[0].name != "1" {
...
func (kl *simpleFakeKubelet) syncTerminatingPod(pod *v1.Pod, podStatus *kubecontainer.PodStatus, runtimeOnly bool, gracePeriod *int64, podStatusFn func(*v1.PodStatus)) error {
	return nil
}
