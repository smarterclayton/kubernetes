...
// returns without error, the pod can be safely cleaned up.
func (kl *Kubelet) syncTerminatingPod(pod *v1.Pod, podStatus *kubecontainer.PodStatus, isRuntimeOnly bool, gracePeriod *int64, podStatusFn func(*v1.PodStatus)) error {
	...
	// refresh the pod status if this is a real pod
	if !isRuntimeOnly {
		apiPodStatus := kl.generateAPIPodStatus(pod, podStatus)
		if podStatusFn != nil {
	...
	if err := kl.killPod(pod, nil, podStatus, gracePeriod); err != nil {
		kl.recorder.Eventf(pod, v1.EventTypeWarning, events.FailedToKillPod, "error killing pod: %v", err)
	...
	// read the status of terminated containers before they get cleaned up - this is inherently racy and may point
	// to more significant changes we need to make
	// TODO: this must be a read through from the pleg cache, otherwise when we remove the containers next we'll lose
	// their status
	...
	// TODO: this is probably broken, but for testing this should be sufficient
