...
func (kl *Kubelet) PodResourcesAreReclaimed(pod *v1.Pod, status v1.PodStatus) bool {
	...
	// // pod's containers should be deleted are allowed to be removed after the pod terminates
...
func (kl *Kubelet) HandlePodCleanups() error {
	...
	for _, runningPod := range runningRuntimePods {
		switch workingPods[runningPod.ID] {
		case kubetypes.SyncPodKill, kubetypes.SyncPodTerminated:
			// if we're already shutting this pod down, we don't need to re-add it
			continue
		default:
			// If the pod isn't in the set that should be running and isn't already terminating, start it
			// This termination is aggressive because we don't already have a termination request (i.e.
			// a deleting static pod should already be terminating). If a static pod is removed from config
			// and the kubelet restarts, it will be aggressively killed here.
	...
	// running according to the pod worker.
	if kl.cgroupsPerQOS {
		pcm := kl.containerManager.NewPodContainerManager()
