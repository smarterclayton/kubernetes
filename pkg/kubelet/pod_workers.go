...
type syncTerminatingPodFnType func(pod *v1.Pod, podStatus *kubecontainer.PodStatus, isRuntimeOnly bool, gracePeriod *int64, podStatusFn func(*v1.PodStatus)) error
...
func (p *podWorkers) managePodLoop(podUpdates <-chan UpdatePodOptions) {
	...
	for update := range podUpdates {
		...
		err := func() error {
			...
			var err error
			if update.UpdateType == kubetypes.SyncPodTerminated || update.RunningPod != nil {
				// once a pod exits terminating with success there are no running containers and no
				// new containers that will be created, so waiting for new cache status doesn't make
				// sense
			...
			case update.UpdateType == kubetypes.SyncPodKill:
				...
				err = p.syncTerminatingPodFn(update.Pod, status, update.RunningPod != nil, gracePeriod, podStatusFn)
