...
func TestGetEtcdData(t *testing.T) {
	...
	NewConfigSourceEtcd("/registry/hosts/machine/kubelet", fakeClient, time.Millisecond, ch)
...
func TestGetEtcdNoData(t *testing.T) {
	...
	c := ConfigSourceEtcd{"/registry/hosts/machine/kubelet", fakeClient, ch, time.Millisecond}
	_, err := c.extractFromResponse(0)
	expectError(t, err)
...
func TestGetEtcd(t *testing.T) {
	...
	c := ConfigSourceEtcd{"/registry/hosts/machine/kubelet", fakeClient, ch, time.Millisecond}
	lastIndex, err := c.extractFromResponse(0)
	expectNoError(t, err)
	if lastIndex != 1 {
...
func TestWatchEtcd(t *testing.T) {
	...
	c := ConfigSourceEtcd{"/registry/hosts/machine/kubelet", fakeClient, ch, time.Millisecond}
	lastIndex, err := c.extractFromResponse(1)
	expectNoError(t, err)
	if lastIndex != 2 {
