...
func TestExtractFromNonExistentFile(t *testing.T) {
	...
	c := ConfigSourceFile{"/some/fake/file", ch}
...
func TestUpdateOnNonExistentFile(t *testing.T) {
	...
	NewConfigSourceFile("random_non_existent_path", time.Millisecond, ch)
	select {
	case got := <-ch:
...
func TestReadFromFile(t *testing.T) {
	...
	ch := make(chan interface{})
	NewConfigSourceFile(file.Name(), time.Millisecond, ch)
	select {
	case got := <-ch:
		update := got.(kubelet.PodUpdate)
		expected := CreatePodUpdate(kubelet.SET, kubelet.Pod{
			Name: fmt.Sprintf("file-%s", filepath.Base(file.Name())),
...
func TestExtractFromBadDataFile(t *testing.T) {
	...
	ch := make(chan interface{}, 1)
	c := ConfigSourceFile{file.Name(), ch}
	err := c.extractFromPath()
	expectError(t, err)
...
func TestExtractFromValidDataFile(t *testing.T) {
	...
	expected := CreatePodUpdate(kubelet.SET, kubelet.Pod{Name: fmt.Sprintf("file-%s", filepath.Base(file.Name())), Manifest: manifest})
	if !reflect.DeepEqual(expected, update) {
		t.Errorf("Expected %#v, Got %#v", expected, update)
	}
}
...
func TestExtractFromDir(t *testing.T) {
	...
	expected := CreatePodUpdate(
		kubelet.SET,
		kubelet.Pod{Name: fmt.Sprintf("file-%s", filepath.Base(files[0].Name())), Manifest: manifests[0]},
		kubelet.Pod{Name: fmt.Sprintf("file-%s", filepath.Base(files[1].Name())), Manifest: manifests[1]},
	)
	if !reflect.DeepEqual(expected, update) {
		t.Errorf("Expected %#v, Got %#v", expected, update)
	}
