...
func TestURLErrorNotExistNoUpdate(t *testing.T) {
	...
	NewConfigSourceURL("http://localhost:49575/_not_found_", time.Millisecond, ch)
	select {
	case got := <-ch:
...
func TestExtractFromHttpBadness(t *testing.T) {
	...
	c := ConfigSourceURL{"http://localhost:49575/_not_found_", ch}
...
func TestExtractFromHttpSingle(t *testing.T) {
	...
	ch := make(chan interface{}, 1)
	c := ConfigSourceURL{testServer.URL, ch}

	err = c.extractFromURL()
	expectNoError(t, err)
	update := (<-ch).(kubelet.PodUpdate)
	expected := CreatePodUpdate(kubelet.SET, kubelet.Pod{Name: "url-", Manifest: manifests[0]})
...
func TestExtractFromHttpMultiple(t *testing.T) {
	...
	update := (<-ch).(kubelet.PodUpdate)
	expected := CreatePodUpdate(kubelet.SET, kubelet.Pod{Name: "url--1", Manifest: manifests[0]}, kubelet.Pod{Name: "url--2", Manifest: manifests[1]})
	if !reflect.DeepEqual(expected, update) {
		t.Errorf("Expected: %#v, Got: %#v", expected, update)
	}
}
