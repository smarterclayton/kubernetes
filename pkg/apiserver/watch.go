...
func serveWatch(watcher watch.Interface, scope RequestScope, req *restful.Request, res *restful.Response, timeout time.Duration) {
	...
	// TODO: store content type on runtime.Unknown and serialize that into the stream, allowing for mixed payloads
	...
	server := &WatchServer{
		...
		fixup: func(obj runtime.Object) {
			if err := setSelfLink(obj, req, scope.Namer); err != nil {
				util.HandleError(fmt.Errorf("failed to set link for object %v: %v", reflect.TypeOf(obj), err))
			}
		},
	...
	for {
		...
		case event, ok := <-s.watching.ResultChan():
			...
			if err := s.embeddedEncoder.EncodeToStream(obj, buf); err != nil {
				// unexpected error
				util.HandleError(fmt.Errorf("unable to encode watch object: %v", err))
				return
			}
			event.Object = &runtime.Unknown{RawJSON: buf.Bytes()}
	...
	for {
		...
		case event, ok := <-s.watching.ResultChan():
			...
			if err := s.encoder.EncodeToStream(&internalEvent, streamBuf); err != nil {
				// encoding error
				util.HandleError(fmt.Errorf("unable to encode event: %v", err))
				s.watching.Stop()
				return
			}
			if s.useTextFraming {
