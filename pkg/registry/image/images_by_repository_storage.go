...
func mappingFromID(id string) (*api.ImageRepositoryMapping, error) {
	...
	return &api.ImageRepositoryMapping{
		...
		Image:          api.Image{JSONBase: api.JSONBase{ID: segments[1]}},
	}, nil
}

func mappingToID(mapping *api.ImageRepositoryMapping) string {
...
// Extract deserializes user provided data into an api.ImageRepository.
func (s *ImagesByRepositoryRegistryStorage) Extract(body []byte) (interface{}, error) {
	result := api.ImageRepositoryMapping{}
