	...
	for _, p := range pods {
		...
		for _, t := range []struct {
			...
			for resourceName, val := range t.total {
				...
				recordMetricWithUnit(ch, t.desc.total, resourceName, unitName, val, p.Namespace, p.Name, p.Spec.NodeName)
				recordMetricWithUnit(ch, t.desc.running, resourceName, unitName, currentVal, p.Namespace, p.Name, p.Spec.NodeName)
			}
		}
	}
}
...
func recordMetricWithUnit(ch chan<- metrics.Metric, desc *metrics.Desc, resourceName v1.ResourceName, unit string, val resource.Quantity, namespace, name, nodeName string) {
	if !val.IsZero() {
		ch <- metrics.NewLazyConstMetric(desc, metrics.GaugeValue,
			val.AsApproximateFloat64(),
			namespace, name, nodeName, string(resourceName), unit,
		)
	}
}
