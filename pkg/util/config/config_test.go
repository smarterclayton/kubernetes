...
func TestConfigurationChannels(t *testing.T) {
	mux := NewMux(nil)
	channelOne := mux.SourceChannel("one")
	if channelOne != mux.SourceChannel("one") {
		t.Error("Didn't get the same muxuration channel back with the same name")
	}
	channelTwo := mux.SourceChannel("two")
...
func TestSimultaneousMerge(t *testing.T) {
	...
	source := mux.SourceChannel("one")
	source2 := mux.SourceChannel("two")
	source <- "test"
	source2 <- "test2"
	<-ch
	<-ch
